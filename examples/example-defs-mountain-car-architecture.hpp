...
Pos normalize_pos(Pos pos)
{
    return ((pos - Simulator::param_type::minPosition()) /
        (Simulator::param_type::maxPosition() - Simulator::param_type::minPosition()));
}
template <typename Speed>
Speed normalize_speed(Speed speed)
{
    return ((speed - Simulator::param_type::minSpeed()) /
...
void phi_direct(gsl_vector *phi, const S &s, const A &a)
{
    ...
    gsl_vector_set(phi, 0/*i*/, normalize_pos(s.position)/*x*/); // sets i-th element value of vector phi to x
    gsl_vector_set(phi, 1/*i*/, normalize_speed(s.speed)/*x*/);
...
#define SPLIT 5
#define SIGMA (1./((SPLIT)-1.))  // 1/4
#define SIGMA2 ((SIGMA)*(SIGMA)) // 1/8
...
#define PHI_RBF_DIMENSION ((SPLIT*SPLIT+1)*3) // 26*3
...
public:
    ...
    void operator()(gsl_vector *phi, const S &s, const A &a) const
    {
        if (phi == nullptr)
        ...
        {
            double dposition = s.position - position[i];
            dposition *= dposition;
            dposition /= /*2 **/ SIGMA2 * DELTA_POSITION2;
            for (int j = 0; j < SPLIT; ++j, ++k)
            {
                double dspeed = s.speed - speed[j];
                dspeed *= dspeed;
