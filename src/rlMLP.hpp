...
public:
	...
	void operator()(const gsl_vector* theta, const state_type& s, const action_type& a, double* y) const
    {
	  phi(xx,s,a);
	  double *iter = y, *end = y + layerSize();
...
public:
	...
	void operator()(const gsl_vector *theta, const state_type &s, const action_type &a, double *y) const
    {
	  ...
	  yyend = yy.end();
	  for(double *i = y; i != yend; ++i)
      {
          double sum = gsl_vector_get(theta,k);++k;
